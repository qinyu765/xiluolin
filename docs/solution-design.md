...
- 当前默认采用可配置 Provider，ASR 支持智谱、OpenAI-compatible、Qwen 和本地 Whisper，文本整理支持智谱、OpenAI-compatible 与 Qwen，同时保持模型名和接口地址可配置。
...
- 录音状态展示；当前录音主要由全局快捷键触发，首页不创建录音控制器。
- 人格选择、默认人格选择和自定义人格管理。
- 识别结果、整理结果和轻量编辑确认。
- 历史记录列表。
- 数据统计卡片。
...
1. 用户打开应用。
2. 用户配置 ASR Provider，包括 API Key、Base URL 和模型名；也可以选择已下载的本地 Whisper。
3. 用户配置文本整理 Provider，包括 API Key、Base URL、模型名和必要生成参数。
4. 用户选择默认人格。
...
切换模式适合不方便持续按住快捷键的短段口述；仍受应用录音 28 秒自动停止限制。
...
自动粘贴失败时，系统将结果复制到剪贴板，并在悬浮窗显示“结果已复制”提示，用户可切回目标应用手动粘贴。当前版本不打开独立结果窗口，也不改变正常成功输入路径。
...
- 返回录音时长。
- 通知前端录音中、处理中、失败等状态。
- 使用设置指定的麦克风，设备失效时回退默认输入；多声道平均下混为单声道。
- 25 秒提示剩余时间，28 秒自动停止；停止音频流后再封装 WAV。
...
- 支持长按模式。
- 支持切换模式。
- macOS 可显式开启独立 Fn 按住录音；短于 300ms 的手势取消，组合修饰键放行。
- 发生快捷键冲突时给出提示。

当前版本提供默认组合快捷键，并在设置页允许用户修改。独立 Fn 依赖辅助功能权限且默认关闭；注册失败不影响组合快捷键，也不修改系统 Fn 偏好。
...
- `AsrRequest`：音频文件、启用热词和可选上一段上下文。
- Provider 配置：API Key、Base URL 和模型名。
...
人格另有 `processing_mode = verbatim | polish`。内置“原文听写”使用 `verbatim`，完全跳过文本模型；其他现有和升级人格默认使用 `polish`。
...
热词词典用于减少专有名词、人名、项目名、技术词的误识别，同时会改变 ASR 的候选偏置；相似词可能互相竞争。

热词设计为本地词典：

- 用户可以新增、编辑、删除热词。
- 热词在 ASR 前读取一次结构化快照，并按 Provider 能力注入识别请求；完整列表继续作为快速文本模型上下文，原文听写也受 ASR 热词影响。
...
`models_ready` 供兼容的外部音频处理命令使用；`can_process` 额外要求麦克风；`can_dictate` 再额外要求快捷键。自动粘贴是非阻断能力，失败时继续使用剪贴板兜底。配置保存或本地模型变化后，由设置页显式增加 refresh revision 刷新检查，也支持用户手动重新检查；不周期轮询系统凭据库。
...
- 首次启动后可以从 Provider catalog 配置 ASR 与文本整理服务。
- 用户可以选择默认人格。
- 用户可以创建自定义人格。
- 用户可以完成一次短语音输入。
- 系统可以返回选定 ASR Provider 的原始识别文本。
- 系统可以根据当前人格生成整理后的文本。
- 用户可以复制最终文本。
- 自动粘贴失败时复制结果仍可用。
- 历史记录成功保存。
- 统计卡片随历史记录更新。
- 热词可以按 Provider 能力参与 ASR，并作为上下文参与文本整理。
- API Key 不出现在 Git 跟踪文件中。

当前快捷键短语音路径已接通；真实 Provider、物理 Fn、麦克风和跨应用输出仍需在目标桌面环境按 [`asr-quality-evaluation.md`](./asr-quality-evaluation.md) 验收。

## 12. 后续扩展

后续迭代可在不推翻当前架构的前提下扩展：

- 持续优化已经实现的本地离线 Whisper Base Q5_1 ASR（云端降级默认关闭）。
- 更多云端 ASR Provider。
- 本地 LLM 或私有化文本模型。
- 人格导入导出。
- 热词批量导入导出。
...
当前项目选择 Tauri + React 桌面助手，本地编排语音输入流程，通过 Provider catalog 接入智谱、OpenAI-compatible、Qwen 和本地 Whisper 能力。系统通过人格配置实现不同场景下的文本整理，通过热词、历史记录和统计卡片增强专业场景可用性和长期使用价值。
