...
- 应用录音统一在 25 秒提示并于 28 秒自动停止；超过 30 秒的外部音频会在请求前被拒绝，具体云端 Provider 也可能有更严格的服务限制。
...
- 处理失败：展示错误约 4 秒；自动输入失败时，结果已复制到剪贴板，悬浮窗提示用户切回目标应用手动粘贴。

状态条在 Windows 和 macOS 上使用透明窗口；其音频条是处理状态动画，不代表麦克风的实时音量。macOS 透明窗口依赖 Tauri 私有 API，因此当前安装包不以 Mac App Store 分发为目标。

### 3. 输出、历史与统计

- 确认结果能够恢复录音开始时的目标应用窗口并写入；精确窗口不可用时可降级为原应用，自动输入失败时复制到剪贴板并提示用户手动粘贴。
- 当前版本不打开独立结果窗口；正常成功输入和自动输入失败都通过悬浮窗状态与剪贴板反馈结果。
...
- 自动输入失败时，确认悬浮窗提示“结果已复制”，切回目标应用使用 `Command+V`；Windows 使用 `Ctrl+V`。
