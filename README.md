...
- 当前阶段：MVP 核心能力已进入联调验证阶段
- 工作分支：`dev`
- 已完成：Tauri v2 + React 基础骨架、本地数据层、内置人格与自定义人格、热词词典、智谱 ASR Provider、OpenAI 文本整理 Provider、短音频处理流程、历史记录、统计卡片、录音模块、全局快捷键、复制与自动粘贴、错误提示、左侧导航、人格管理页、热词页和设置页
- 当前界面：采用左侧导航结构，包含首页、人格、热词、设置四个页面；首页当前聚焦当前人格问候、快捷键提示、统计卡片和时间分段历史记录
- 待完成验证：UI 重构端到端验证（T028）、真实 API Key 和样例音频下的 ASR + OpenAI smoke test、快捷键录音触发后的桌面端完整联调
...
9. 执行短语音输入流程后，确认结果可复制或输出，历史记录和统计数据随之更新。

当前 T028 仍在待验证状态。若真实 API Key、麦克风权限或样例音频不可用，可以先演示配置、人格管理、热词管理、历史和统计页面，再补充真实服务 smoke test 结果。
