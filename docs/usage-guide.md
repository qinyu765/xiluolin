...
- 设置页会自动保存修改：开关、下拉和快捷键立即保存，文本与 API Key 停止输入约 600 毫秒后保存，失焦时立即提交。
- 顶部显示“已保存”后，先用短语音验证 ASR，再验证文本整理和跨应用输出，便于定位失败环节；不要直接编辑应用数据文件或把密钥写入仓库。
...
- 处理失败：显示红色失败状态，短暂停留后自动消失；自动输入失败时，完整结果和恢复操作在独立结果窗口中保留。
