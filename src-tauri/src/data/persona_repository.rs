...
impl LocalDatabase {
    ...
    pub fn delete_persona(&self, id: &str) -> Result<(), String> {
        let transaction = self
            ...
            .map_err(|error| error.to_string())?;
        delete_persona_from_connection(&transaction, id)?;
        transaction.commit().map_err(|error| error.to_string())
    }

    pub fn delete_default_persona_with_persistence(
        &self,
        id: &str,
        persist_config: impl FnOnce() -> Result<(), String>,
        ...
        delete_persona_from_connection(&transaction, id)?;
        let personas =
            list_personas_from_connection(&transaction).map_err(|error| error.to_string())?;

        if let Err(error) = persist_config() {
            return Err(error);
        }

        if let Err(error) = transaction.commit() {
            return match restore_config() {
                Ok(()) => Err(format!("人格删除提交失败，配置已回滚：{error}")),
                Err(restore_error) => Err(format!(
                    "人格删除提交失败且配置回滚失败：{error}; {restore_error}"
                )),
            };
        }
...
fn delete_persona_from_connection(
    ...
    if is_default {
        let general_exists = connection
            ...
            .is_some();
        if !general_exists {
            return Err("通用人格不存在，无法完成删除".to_string());
        }
        connection
