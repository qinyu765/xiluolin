...
- 智谱会接收前 100 个启用且稳定去重的热词；OpenAI 和本地 Whisper 只把热词作为软提示。
...
需要严格保留口述原文时，将默认人格切换为“原文听写”。该模式跳过文本模型，只清理首尾和异常空白；历史记录会明确标记为 `verbatim`。
...
- 第一阶段不支持边说边出字；智谱的文件上传流式响应不等于实时麦克风流。
