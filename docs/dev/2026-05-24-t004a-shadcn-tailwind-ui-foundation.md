...
- 评估 `https://getdesign.md/hp/design-md` 的 HP 风格，决定不完整注入。
- 选择 `https://getdesign.md/notion/design-md` 作为更适合当前桌面效率工具的视觉参考，并补充 `docs/design/ui-style.md`。
...
HP design-md 的完整风格更偏官网和产品目录。当前项目是桌面效率工具，因此改为采用更贴近 workspace tools 的 Notion 风格，并只落地基础 token，避免让主界面变成营销页。
...
- `pnpm build`：通过。Vite 构建成功，生成 CSS gzip 约 6.83 kB，JS gzip 约 100.50 kB。
