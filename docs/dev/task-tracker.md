...
> 当前工作方式：以 `main` 为唯一日常开发基线；常规任务直接在 `main` 上完成修改、验证、提交和推送，不再要求持久 `dev` 分支或 routine feature branch。提交或推送前必须先向用户展示改动与验证结果并获得明确同意；仅在用户要求、外部贡献或高风险隔离审查时使用 PR。历史任务中的 `dev` 记录保持不变。
...
1. 常规任务直接在 `main` 上修改和验证。
2. 提交、推送或创建 PR 前，必须展示改动文件、验证结果、建议提交信息和交付方式，并获得用户明确确认。
3. 用户确认后，将小而完整的提交直接推送到 `origin/main`。
4. 仅在用户明确要求、外部贡献或高风险改动需要隔离审查时创建临时分支和 PR；PR 必须以 `main` 为目标分支。
...
| Done | 已推送到 `main`、PR 已合入或经用户明确确认完成 |
...
| T028A | P0 | 切换 main 直接开发流程 | 将 `dev` 未合入改动通过 PR #21 rebase 合入 `main`，并把当前协作规范统一调整为直接基于 `main` 开发 | Review | main | 不适用（main 直提） | 文档引用检查、规则一致性检查、`git diff --check` | [2026-07-17-t028a-main-development-workflow.md](./2026-07-17-t028a-main-development-workflow.md) | PR #21 已合入；工作流文档已更新，等待审查后提交到 `main` |
| T029 | P0 | 建立前后端质量检查基线 | 修复现有 TypeScript 与 Rust 编译阻塞，建立统一检查脚本和 Windows CI | Review | main | 不适用（main 直提） | `pnpm install --frozen-lockfile`、`pnpm check`、Windows CI | [2026-07-17-t029-quality-security-baseline.md](./2026-07-17-t029-quality-security-baseline.md) | 本地完整检查通过；等待提交并确认 Windows CI |
